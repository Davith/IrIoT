...
static void hrSensorDataInit(void)
{
    ...
    TimerDelete(g_hr_data.bonding_reattempt_tid);
    g_hr_data.bonding_reattempt_tid = TIMER_INVALID;
...
static uint8* receivedHRMeasReading(uint8 *p_length)
{
    ...
    if(num_of_rr_val)
    {
        uint8 rr_val_arr[2*MAX_RR_VALUES];
                               /* Temporary array to store the RR values. These
...
static void handleSignalGattConnectCfm(GATT_CONNECT_CFM_T* p_event_data)
{
    ...
    switch(g_hr_data.state)
    {
        ...
        case app_state_slow_advertising:
        {
            if(p_event_data->result == sys_status_success)
            {
                /* Boolean flag to indicate whether the connected address 
                 * matches with the bonded address
                 */
                bool bonding_exists = FALSE;
                    ...
                    if((GattIsAddressResolvableRandom
                       ...
                       (SMPrivacyMatchAddress(&p_event_data->bd_addr,
                            ...
                            (!GattIsAddressResolvableRandom
                             (&g_hr_data.bonded_bd_addr) &&
                               (!MemCmp(&g_hr_data.bonded_bd_addr,
                                        &g_hr_data.con_bd_addr,
                                        sizeof(g_hr_data.bonded_bd_addr)))))
                    {
                        bonding_exists = TRUE;
                    }
                    /* Initilaise service data */
                    HeartRateOnConnection(bonding_exists);
                    BatteryOnConnection(bonding_exists);
                    
                    /* If we are bonded to this host, then it may be appropriate
                     * to indicate that the database is not now what it had
                     * previously.
                     */
                    GattOnConnection(p_event_data->cid,bonding_exists);
                }
                                 ...
                                 SM_SIMPLE_PAIRING_COMPLETE_IND_T *p_event_data)
{
    ...
    switch(g_hr_data.state)
    {
        case app_state_connected:
        {
            ...
            {
                if(g_hr_data.bonding_reattempt_tid != TIMER_INVALID)
                {  
                    /* Delete the bonding chance timer */
                    TimerDelete(g_hr_data.bonding_reattempt_tid);
                    g_hr_data.bonding_reattempt_tid = TIMER_INVALID;
            ...
            else
            {
                 ...
                 */
                 ...
                 else if(g_hr_data.bonded)
                 {
                    g_hr_data.bonding_reattempt_tid = TimerCreate(
                    ...
                    HCI_EV_DATA_ENCRYPTION_CHANGE_T *p_event_data)
{
    ...
    switch(g_hr_data.state)
    {
        case app_state_connected:
        {
            if(p_event_data->enc_enable == FALSE)
            {
                /* Encryption failed */
...
static void handleSignalLsRadioEventInd(void)
{
     ...
     * for the last sent notification.
     *
     * CSR101x Boards:
    ...
    if((g_hr_data.conn_interval*1250UL) == (uint16)HR_MEAS_TIME)
    {
       ...
       * when PREFERRED_MAX_CON_INTERVAL is same as HR_MEAS_TIME. In this case,
       * only the condition for "if" part will get executed.
       */
                ...
                else if(p_event_data->reason == HCI_ERROR_CONN_TERM_LOCAL_HOST)
                {
                    ...
                    else
                    {
...
#ifndef NO_ACTUAL_MEASUREMENT
                        /* With actual measurements, the application runs an
                         * idle timer. If that timer expires(app_tid will be
                         * invalid in that case), the application disconnects
                         * the link and should go into Dormant mode.
                         */
                        if(g_hr_data.app_tid == TIMER_INVALID)
                        {
#ifdef ENABLE_DORMANT_MODE_FUNCTIONALITY
                            /* The Idle timer is not running. This means the
                             * application has disconnected the link on idle
                             * timer expiry. Put the HR sensor into Dormant
                             * mode.The application is expecting the Wake pin to
                             * be in shorted state with the HR Input Pio. So
                             * the second parameter shall be set in accordance
                             * with the defaultpull mode on the HR input pio. If
                             * it is pulled up,we need to configure wake on low
                             * value(2nd parameter)Note: Third parameter gets
                             * ignored in Dormant state.
                             */
                        ...
                        else
                        {
                            ...
                            if(g_hr_data.bonded)
                            {
                                /* If the device is bonded and host uses
                                 * resolvable random address, the device
                                 * initiates disconnectprocedure if it gets
                                 * reconnected to a different host,in which case
                                 * device should trigger fastadvertisements
                                 * after disconnecting from the last connected
                                 * host.
                                 */
                                if(GattIsAddressResolvableRandom(
                                                   &g_hr_data.bonded_bd_addr) &&
                                ...
                                {
                                    /* Else move to app_state_idle state because
                                     * of inactivity
                                     */
                                    AppSetState(app_state_idle);
                                }
...
static void handleBondingChanceTimerExpiry(timer_id tid)
{
    if(g_hr_data.bonding_reattempt_tid== tid)
    {
        g_hr_data.bonding_reattempt_tid= TIMER_INVALID;
...
extern void AppProcessSystemEvent(sys_event_id id, void *data)
{
    switch(id)
    {
        case sys_event_battery_low:
        {
            ...
            if(g_hr_data.state == app_state_connected)
            {
                BatteryUpdateLevel(g_hr_data.st_ucid,TRUE);
            }
        }
        break;
