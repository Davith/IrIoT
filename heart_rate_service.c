...
/*----------------------------------------------------------------------------*
 *  NAME
 *      HeartRateOnConnection
 *
 *  DESCRIPTION
 *      This function initialises HR service data on connection
 *
 *  RETURNS
 *      Nothing.
 *
...
extern void HeartRateOnConnection(bool bonding_exists)
{
    if(bonding_exists)
    {
        /* Read heart rate client configuration characteristic descriptor from 
         * NVM if connected device is same as bonded device
         */
        Nvm_Read((uint16*)&g_hr_serv_data.hr_meas_client_config,
                 sizeof(g_hr_serv_data.hr_meas_client_config),
                 g_hr_serv_data.nvm_offset +
    ...
    {
        /* Initialise heart rate client configuration characterisitic
