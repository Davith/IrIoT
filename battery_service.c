...
extern void BatteryHandleAccessRead(GATT_ACCESS_IND_T *p_ind)
{
    ...
    switch(p_ind->handle)
    {
        case HANDLE_BATT_LEVEL:
        {
            ...
            {
                g_batt_data.level = BATTERY_CRITICAL_LEVEL;
            }
            else
            {
...
extern bool CheckLowBatteryVoltage(void)
{
    ...
    if(BatteryReadVoltage() <= BatteryReadLowThreshold())
    {
         ...
         * triggers notifications on reading the current battery level
         */
        g_batt_data.level = 0xFF;
        BatteryUpdateLevel(AppGetConnectionCid(),low_battery);
    }
    return low_battery;    
}
 ...
 *
 *  DESCRIPTION
 *      This function initialises Battery service data on connection
 *
 *  RETURNS
 *      Nothing.
 *
...
extern void BatteryOnConnection(bool bonding_exists)
{
    if(bonding_exists)
    {
       /* Read battery level client configuration descriptor from NVM if 
        * connected device is same as bonded device
        */
        Nvm_Read((uint16*)&g_batt_data.level_client_config,
                sizeof(g_batt_data.level_client_config),
                g_batt_data.nvm_offset + 
    ...
    {
        /* Initialise battery level client configuration characterisitic
         * descriptor value if connected device is different from bonded device
         */
        g_batt_data.level_client_config = gatt_client_config_none;
    }
}
